...
def is_thumbnail_or_icon(url: str, width: Optional[int] = None, height: Optional[int] = None) -> bool:
    """Check if image is likely a thumbnail or icon based on URL and dimensions."""
    url_lower = url.lower()
    
    # Check URL patterns
    thumbnail_patterns = [
        'thumb', 'thumbnail', 'icon', 'small', 'tiny', 
        'avatar', 'logo', 'badge', 'button'
    ]
...
def is_product_image(img_element, url: str) -> bool:
    ...
    for indicator in ['product', 'item', 'shop', 'buy', 'price', '$']:
        if indicator in img_classes.lower() or indicator in img_id.lower() or indicator in img_alt:
            return True
